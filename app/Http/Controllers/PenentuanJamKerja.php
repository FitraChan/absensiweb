...
class PenentuanJamKerja extends Controller
{
  public function penentuan_jam_kerja($karyawan_id, $waktu_kerja, $date)
  {
    ...
    $tanggal_kemarin = date('Y-m-d', strtotime('-1 days', strtotime($date)));
    ...
    if ($waktu_kerja == 1) {
      ...
      $set_group_jadwal = DB::table('tb_jam_kerja')->select(DB::raw('HOUR(waktu_mulai) as waktu_mulai'))->whereIn('id', function ($query) use ($group_jadwal) {
        ...
        $query->select('jam_kerja_id')->from('detail_group_jadwals')->where('group_jadwal_id', $id_kar);
      })->get();
      ...
      if (empty($absen_morning->jam_kerja_id)) {
        $cari_absen_terakhir = DB::table('tb_absensi')->select('jam_masuk', 'jam_kerja_id', 'keterangan')->where('karyawan_id', $karyawan_id)->where('tanggal', $tanggal_kemarin)->whereTime('jam_masuk', '>', '16:00:00')->orderBy('id', 'desc')->first();
      }


      $jam_kerja_morning = !empty($absen_morning->jam_kerja_id) ? DB::table('tb_jam_kerja')->where('id', $absen_morning->jam_kerja_id)->first() : DB::table('tb_jam_kerja')->where('id', $cari_absen_terakhir->jam_kerja_id)->first();
...
//print_r("$index_num ".$index_num.'  -  '.'$index_waktu_akhir  '.$index_waktu_akhir);
      if ($index_num  > $index_waktu_akhir) {
        ...
        if (empty($absen_morning->jam_kerja_id)) {
          ...
          $cari_absen_terakhir = DB::table('tb_absensi')->select('jam_masuk', 'jam_kerja_id', 'keterangan')->where('tanggal', $tanggal_kemarin)->where('karyawan_id', $karyawan_id)->whereTime('jam_masuk', '>', '16:00:00')->orderBy('id', 'desc')->first();
        }
        ...
        $waktu_akhir_shift_masuk = !empty($absen_morning->jam_kerja_id) ? DB::table('tb_jam_kerja')->where('id', $absen_morning->jam_kerja_id)->first() : DB::table('tb_jam_kerja')->where('id', $cari_absen_terakhir->jam_kerja_id)->first();
        ...
        $data['jam_kerja_id'] = !empty($absen_morning->jam_kerja_id) ? $absen_morning->jam_kerja_id : $cari_absen_terakhir->jam_kerja_id;

        // print_r('absen_morning->jam_kerja_id: ' . $cari_absen_terakhir->jam_kerja_id);
